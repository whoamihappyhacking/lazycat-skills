...
description: 懒猫微服(Lazycat MicroServer)应用开发的终极总控指南。当用户提出任何与懒猫微服应用开发、打包(lpk)、路由配置、部署参数、认证体系(OIDC)或SDK调用相关的需求时触发。
---
...
**适用场景：** 开发者从零开始写代码（Go 或 Node.js），需要调用 `@lazycatcloud/sdk` 与底层系统交互，或者需要注册文件扩展名关联（`file_handler`）让网盘双击打开应用。
**行动指令：** 请读取并遵循 `references/sdk-dev.md` 中的规范。

### 6. 应用上架商店与发布 (Store Publishing)
**适用场景：** 开发者已经完成应用的开发和测试，需要将应用上架到懒猫应用商店，或者需要了解商店审核规则、镜像推送到官方仓库的流程、社区激励规则等。
